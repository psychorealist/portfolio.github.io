...
let loco;
if (container) {
    loco = new (0, _locomotiveScrollDefault.default)({
        el: container,
        smooth: true,
        multiplier: 1,
        getDirection: true
    ...
    console.log("Locomotive running");
}
console.log("MAIN JS LOADED");
/* =========================================================
   HIDE MENU + HOME ON SCROLL DOWN
   SHOW ON SCROLL UP
   LOCOMOTIVE COMPATIBLE
========================================================= */ const menu = document.querySelector(".menu");
const homeButton = document.querySelector(".home");
let lastScrollY = 0;
if (loco && menu && homeButton) loco.on("scroll", (args)=>{
    const currentScroll = args.scroll.y;
    // scrolling down
    if (currentScroll > lastScrollY && currentScroll > 100) {
        menu.classList.add("hide-menu");
        homeButton.classList.add("hide-menu");
    } else {
        menu.classList.remove("hide-menu");
        homeButton.classList.remove("hide-menu");
    }
...
if (site_wide_cursor) {
    document.addEventListener('mousemove', (evt)=>{
        // Make cursor visible as soon as Safari detects movement
        site_wide_cursor.style.display = 'block';
        const w = site_wide_cursor.offsetWidth;
        const h = site_wide_cursor.offsetHeight;
        const x = evt.clientX - w / 2;
        const y = evt.clientY - h / 2;
        site_wide_cursor.style.webkitTransform = `translate3d(${x}px, ${y}px, 0)`;
        site_wide_cursor.style.transform = `translate3d(${x}px, ${y}px, 0)`;
    });
...
/* =========================================================
   HORIZONTAL SLIDERS
   ========================================================= */ document.querySelectorAll('.imgL, .imgR').forEach((container)=>{
    const scrollContainer = container.querySelector('.scroll-container');
    // Ignore normal images that are not sliders
    ...
    function update() {
        ...
        scrollContainer.style.transform = `translateX(-${currentIndex * w}px)`;
    }
...
if (loco) loco.on('scroll', handleVideoPlayback);
else window.addEventListener('scroll', handleVideoPlayback);
/* =========================================================
   TOUCH SWIPE (SLIDERS)
   ========================================================= */ document.querySelectorAll('.imgL, .imgR').forEach((container)=>{
    let startX = 0;
    ...
    container.addEventListener('touchend', ()=>{
        if (startX - endX > 50) container.querySelector('.arrow.right')?.click();
        else if (endX - startX > 50) container.querySelector('.arrow.left')?.click();
        startX = 0;
        endX = 0;
    });
});
...
const viewerImg = document.getElementById("gallery-image");
const prevBtn = document.querySelector(".gallery-prev");
const nextBtn = document.querySelector(".gallery-next");
const closeBtn = document.querySelector(".gallery-close");
const counter = document.querySelector(".gallery-counter");
const loader = document.querySelector(".gallery-loader");
const images = Array.from(document.querySelectorAll(".container1 img"));
let currentIndex = 0;
let scale = 1;
...
let offsetX = 0;
let offsetY = 0;
/* OPEN */ function openViewer(index) {
    if (!viewer) return;
    currentIndex = index;
...
/* LOAD */ function loadImage() {
    if (loader) viewer.classList.add("gallery-loading");
    ...
    temp.onload = ()=>{
        ...
        if (loader) viewer.classList.remove("gallery-loading");
        resetTransform();
    };
    temp.src = img.src;
}
/* NAV */ function nextImage() {
...
/* COUNTER */ function updateCounter() {
...
/* DRAG */ if (viewerImg) viewerImg.addEventListener("mousedown", (e)=>{
    if (scale <= 1) return;
    isDragging = true;
...
/* EVENTS */ images.forEach((img, index)=>{
...
window.addEventListener("load", ()=>{
    if (loco) setTimeout(()=>{
        ...
        console.log(loco.scroll.instance.limit.y);
    }, 2000);
});

},{"locomotive-scroll":"hVArX","@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}],"hVArX":[function(require,module,exports,__globalThis) {
...
},{}]},["io2N8","bNJxx"], "bNJxx", "parcelRequireca67", {})

//# sourceMappingURL=portfolioWebsite.36960861.js.map
